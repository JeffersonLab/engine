      subroutine h_dc_ntuple_change(ABORT,err)
*----------------------------------------------------------------------
*
...
*----------------------------------------------------------------------
      ...
      character*15 here
      parameter (here='h_dc_ntuple_change')
...
*--------------------------------------------------------
      ...
      h_dc_ntuple_filesegments = h_dc_ntuple_filesegments + 1

      if (h_dc_ntuple_filesegments .le. 9) then
        ifile = char(ichar('0')+h_dc_ntuple_filesegments)
      else
        ifile = char(ichar('a')+h_dc_ntuple_filesegments-10)
      endif
      ...
      IF (.not.ABORT) call h_dc_ntuple_open(file,ABORT,err)
