...
c$$$      endif
      ...
      if (.not.(b_passed_cointime_cut.and.h_passed_cointime_cut ))return

      if(bigcal_ntuple_type.eq.1)then
        if(bigcal_all_nclust_good.gt.0) then
          call b_ntuple_keep(ABORT,err)
        endif
      else if(bigcal_ntuple_type.eq.2)then 
         call b_ntuple_keep(ABORT,err)
      else if(bigcal_ntuple_type.eq.3) then
         if(bigcal_all_nclust_good.gt.0) call b_ntuple_keep(ABORT,err)
      endif
