...
* $Log$
* Revision 1.1  1995/01/10 18:43:44  cdaq
* Initial revision
*
*
	implicit none
...
* Now define the mother volume that the detectors sit in
	...
	call ugsvolu ('HHUT', 'BOX ', HHUTMEDIA, par, 3, ivolu)
...
* Now define the wire chambers as a collection of 6 planes
	...
	call ugsvolu ('WCXP', 'BOX ', DETMEDIA, par, 3, ivolu)	! X plane
	call ugsvolu ('WCYP', 'BOX ', DETMEDIA, par, 3, ivolu)	! Y plane
	call ugsvolu ('WCUP', 'BOX ', DETMEDIA, par, 3, ivolu)	! U plane
	call ugsvolu ('WCVP', 'BOX ', DETMEDIA, par, 3, ivolu)	! V plane

! make a volume for 6 planes
	par(3) = (6./5. * (hdc_zpos(6) - hdc_zpos(1))) / 2.
	call ugsvolu ('WCHA', 'BOX ', DETMEDIA, par, 3, ivolu)	! Wire chamber
...
*
	...
	par(2) = hdc_nrwire(2)*hdc_pitch(2)/ 2. ! half width of chamber planes
	par(3) = (hdc_zpos(2) - hdc_zpos(1))/ 2.! half width of chamber planes
	call ugsvolu ('WCWX', 'BOX ', DETMEDIA, par, 3, ivolu)	! X cell
	...
	call ugsvolu ('WCWY', 'BOX ', DETMEDIA, par, 3, ivolu)	! Y cell
	...
	call ugsvolu ('WCWU', 'BOX ', DETMEDIA, par, 3, ivolu)	! U cell
	...
	call ugsvolu ('WCWV', 'BOX ', DETMEDIA, par, 3, ivolu)	! V cell
...
* Now define the hodoscope layers
	...
	par(3) = HODO_THICKNESS / 2.	! half thickness of hodoscope in z
	call ugsvolu ('HOYS', 'BOX ', DETMEDIA, par, 3, ivolu)	! Y plane hodo

	par(1) = hscin_1x_size / 2.	! half width of X strips
	par(2) = hscin_1y_size * HODO_Y_PADDLES / 2.
						! half width of hodoscope in y
	par(3) = HODO_THICKNESS / 2.	! half thickness of hodoscope in z
	call ugsvolu ('HOXS', 'BOX ', DETMEDIA, par, 3, ivolu)	! X plane hodo
...
! half thickness of hodoscope in z, times 2 because of stagger
	par(3) = HODO_THICKNESS / 2. * 2.
	call ugsvolu ('HODX', 'BOX ', DETMEDIA, par, 3, ivolu)	! X plane hodo
	call ugsvolu ('HODY', 'BOX ', DETMEDIA, par, 3, ivolu)	! Y plane hodo

! box for hodo
	par(3) = (2. * HODO_THICKNESS + (hscin_1y_zpos-hscin_1x_zpos)) / 2.
	call ugsvolu ('HODO', 'BOX ', DETMEDIA, par, 3, ivolu)	! hodoscope box
	...
	z = -(HODO_THICKNESS + (hscin_1y_zpos-hscin_1x_zpos)) / 2.
...
! half height of the shower detector
	...
	call ugsvolu ('SHOW', 'BOX ', DETMEDIA, par, 3, ivolu)
...
! half height of the shower detector
	...
	call ugsvolu ('LAYE', 'BOX ', DETMEDIA, par, 3, ivolu)
	...
	call ugsvolu ('BLOC', 'BOX ', DETMEDIA, par, 3, ivolu)
