...
*     ALTERNATIVE LEFT-RIGHT DETERMINATION ROUTINE BY AJP
      ...
      real*4 HMStrack(4)
      real*4 FPPtrack(4)
      real*4 newfpptrack(4)
      real*4 DCcoords(3), FPcoords(3), fpslopes(3)
      ...
      integer goodcombos(64) ! does this combo pass the chi2 cut?
      ...
      logical firsttry,anyambig,badzclosebest
      ...
      integer*4 ncombos,ngoodcombos,combo
...
c     number of combos passing chi2 cut:
      ngoodcombos = 0
...
*     if chi2 passes the tracking cut, then we have a good combo:
         if(chi2.lt.hfpp_min_chi2) then 
            ngoodcombos = ngoodcombos + 1
            goodcombos(ngoodcombos) = icombo+1
c     first translate tracks to focal plane coord system
            dccoords(1) = testtrack(1)
            dccoords(2) = testtrack(3)
            dccoords(3) = 1.0
            call h_fpp_dc2fp(dcset,.true.,dccoords,fpslopes)
...
*     calc. scattering params:
            ...
            if( ngoodcombos .eq. 1 .or. theta .lt. mintheta ) then
               mintheta = theta
            endif
         endif
...
c     how to handle "ambiguities?"
c     first of all, don't do this if we are going to remove hits and do it again:
c     if we have multiple combos passing the chi2 cut for tracking, we want to 
c     determine whether a different combo will give us a better track:

      if( ntrackpoints.eq.ntrackplanes.and.
     $     ntrackplanes.eq.h_fpp_n_dcinset*h_fpp_n_dclayers .and. 
     $     ngoodcombos.gt.1 .and. hfppfixleftright.gt.0) then
c     questions:
c     1. does the best chi2 combo have a BAD zclose? if NOT, don't change this track
c     2. does the smallest theta combo have an angle consistent with Coulomb, i.e. zero?
c     3. do other combos have zclose values inside the CH2? 
c     PLAN OF ACTION: 

         zmin = hfpp_prune_zclose(2*(dcset-1)+1)
         zmax = hfpp_prune_zclose(2*(dcset-1)+2)

         badzclosebest = abs(combozclose(bestcombo+1)-hfpp_zoff(dcset))
     $        .lt.abs( hfpp_zoff(dcset) - zmax ) .and. 
     $        combotheta(bestcombo+1).gt.hfpp_ambig_smallthetatest

         if(badzclosebest) then
            if(combotheta(bestcombo+1).lt.16./57.3) then
               write(*,*) 'multi-good and bad z'
               write(*,*) 'old z,theta=',combotheta(bestcombo+1),
     $              combozclose(bestcombo+1)
            endif
            firsttry = .true.
            if( mintheta.le.hfpp_ambig_smallthetatest ) then ! choose the smallest theta combo passing the chi2 cut:
               do icombo=1,ngoodcombos
                  combo = goodcombos(icombo) 
                  if( firsttry.or. combotheta(combo).lt.mintheta ) then
                     firsttry = .false.
                     mintheta = combotheta(combo)
                     bestcombo = combo - 1
                  endif
               enddo
            else ! smallest theta is not "small": look for good zclose:
               do icombo=1,ngoodcombos
                  combo = goodcombos(icombo)
                  if(combozclose(combo).lt.zmax.and.
     $                 (firsttry.or.combochi2(combo).lt.minchi2) ) then
                     firsttry = .false.
                     minchi2 = combochi2(combo)
                     bestcombo = combo - 1
                  endif
               enddo
            endif
            if( .not. firsttry ) then
               if( combotheta(bestcombo+1) .lt.16./57.3 ) then
                  write(*,*) 'changed left-right combo'
                  write(*,*) 'new z,theta=',combozclose(bestcombo+1),
     $                 combotheta(bestcombo+1)
               endif
               ifree = 0
               do ipoint = 1,ntrackpoints
                  if( .not. plusminusfixed(ipoint) ) then
                     ifree = ifree + 1
                     if( jbit(bestcombo,ifree).eq.1 ) then
                        plusminusbest(ipoint) = 1.
                     else 
                        plusminusbest(ipoint) = -1.
                     endif
                  endif
