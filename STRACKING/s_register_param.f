...
*
     ...
     &             smax_num_chambers,flag)
       ...
       ABORT= ierr.EQ.0 .or. ABORT
       ierr= regparmreal('sdrift_velocity',sdrift_velocity,flag)
       IF(ierr.NE.0) err= 'unable to register "sdrift_velocity"'
       ABORT= ierr.EQ.0 .or. ABORT
       ierr= regparmint('sluno',sluno,flag)
       IF(ierr.NE.0) err= 'unable to register "sluno"'
       ABORT= ierr.EQ.0 .or. ABORT
       ierr= regparmint('sdebugflagpsi',sdebugflagpsi,flag)
       IF(ierr.NE.0) err= 'unable to register "sdebugflagpsi"'
       ABORT= ierr.EQ.0 .or. ABORT
       ierr= regparmint('sdebugflaggeometry',sdebugflaggeometry,flag)
       IF(ierr.NE.0) err= 'unable to register "sdebugflaggeometry"'
       ABORT= ierr.EQ.0 .or. ABORT
       ierr= regparmint('sdebugflagpr',sdebugflagpr,flag)
       IF(ierr.NE.0) err= 'unable to register "sdebugflagppr"'
       ABORT= ierr.EQ.0 .or. ABORT
       ierr= regparmint('sdebugflagstubs',sdebugflagstubs,flag)
       IF(ierr.NE.0) err= 'unable to register "sdebugflagstubs"'
       ABORT= ierr.EQ.0 .or. ABORT
       ierr= regparmint('sdebuglinkstubs',sdebuglinkstubs,flag)
       IF(ierr.NE.0) err= 'unable to register "sdebuglinkstubs"'
       ABORT= ierr.EQ.0 .or. ABORT
       ierr= regparmint('sdebugtrackprint',sdebugtrackprint,flag)
       IF(ierr.NE.0) err= 'unable to register "sdebugtrackprint"'
       ABORT= ierr.EQ.0 .or. ABORT
       ierr= regparmint('sdebugstubchisq',sdebugstubchisq,flag)
       IF(ierr.NE.0) err= 'unable to register "sdebugstubchisq"'
       ABORT= ierr.EQ.0 .or. ABORT
       ierr= regparmint('sdebugtartrackprint',sdebugtartrackprint,flag)
