...
*     propogation of light thru scintillator, and offset.
     ...
     $               sqrt(max(0.,(adc_ph/sscin_minph-1.)))
...
ccc in s_physics in the "goodtrack" figurer outer.
     ...
     $                     ))
                  ...
                  if (sgood_tdc_neg(hit)) then
                    sdedx(trk,snum_scin_hit(trk))=float(sscin_adc_neg(hit))
                  else
...
**    For now, require at least 3 planes, to avoid bad fits.
          numplanes=0
          if (sgood_plane_time(1)) numplanes=numplanes+1
          if (sgood_plane_time(2)) numplanes=numplanes+1
          if (sgood_plane_time(3)) numplanes=numplanes+1
          if (sgood_plane_time(4)) numplanes=numplanes+1
