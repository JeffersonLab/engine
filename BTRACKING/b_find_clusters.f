...
c$$$         endif
     ...
     $              .and.icol.le.BIGCAL_PROT_NX) then
                  icell = icol + BIGCAL_PROT_NX*(irow - 1)
                  if(BIGCAL_PROT_GOOD_HIT(icell)) then
     ...
     $                 then
               ...
               if(minxdiff .lt. 1000.) then
                  do icol = ixmin - maxcelldiff,ixmin + maxcelldiff
                     icell = icol + BIGCAL_RCS_NX * (irow - 1)
                     if(icol.ge.1.and.icol.le.BIGCAL_RCS_NX.and.
     $                    BIGCAL_RCS_GOOD_HIT(icell)) then
                        ncellclust = ncellclust + 1
         ...
         do ihit=1,ncellclust
            do jhit=ihit+1,ncellclust
               if(clstr_temp_ecell(jhit).gt.clstr_temp_ecell(ihit))then
                  ...
                  ! at this point all goodhit are true, so no need to switch
...
c$$$         endif
     ...
     $              .and.icol.le.BIGCAL_RCS_NX) then
                  icell = icol + BIGCAL_RCS_NX*(irow - 1)
                  if(BIGCAL_RCS_GOOD_HIT(icell)) then
                     ncellclust = ncellclust + 1
 ...
 200     continue
         ...
         ! in the special case of iymax = 34 add closest cells in the first rows of RCS part
     ...
     $                 then
               ...
               if(minxdiff .lt. 1000.) then
                  do icol = ixmin - maxcelldiff,ixmin + maxcelldiff
                     icell = icol + BIGCAL_PROT_NX*(BIGCAL_PROT_NY-irow)
                     if(icol.ge.1.and.icol.le.BIGCAL_PROT_NX.and.
     $                    BIGCAL_PROT_GOOD_HIT(icell)) then
                        ncellclust = ncellclust + 1
 ...
 202     continue
     ...
     $     goto 101
      ...
      !!!!!!!!Come back to the middle section later!!!!!!!!!!!!!!1
